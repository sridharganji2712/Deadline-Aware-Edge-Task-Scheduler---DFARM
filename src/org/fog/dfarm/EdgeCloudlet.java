 ...
 *  - originNode        : Edge node whose wireless link received this task.
 */
public class EdgeCloudlet extends Cloudlet {
    ...
    // DPL formula (paper Eq. 1/15): (deadline - arrivalTime) / MI — a fixed
    // property of the task, computed once, not against whatever the current
    // decision-time clock happens to be. currentTime is kept as a parameter
    // (unused) so existing/future callers don't need a signature change —
    // see DFARMScheduler.networkAwareDPL for the scheduler's own copy of
    // this formula (with an additional edge-specific uplink adjustment).
    public double computeDPL(double currentTime) {
        return (deadline - arrivalTime) / (double) getCloudletLength();
    }
